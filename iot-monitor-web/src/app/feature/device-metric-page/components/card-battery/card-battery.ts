...
@Component({
  selector: 'app-card-battery',
  imports: [MaterialModule, DecimalPipe],
  templateUrl: './card-battery.html',
  styleUrl: './card-battery.css',
})
export class CardBattery implements OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const sub1 = this.deviceMetricData.getMetricByDeviceId(device.id).subscribe((allMetrics) => {
        this.metrics.set(allMetrics.filter((m) => m.metric === 'battery'));
      });
      this.subscriptions.push(sub1);

      // 2. Suscribirse a nuevas métricas vía socket
      const callback = (metric: DeviceMetric) => {
        if (metric.device.id === device.id && metric.metric === 'battery') {
